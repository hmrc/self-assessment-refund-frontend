...
class RefundRejectedControllerSpec extends ItSpec with PageContentTesting {
  ...
  "onPageLoad" should {
    ...
    "render 'Refund Rejected' page with correct content for Agent" in new TestSetup(isAgent = true) {
      ...
      val refundControllerAuth: RefundRejectedController = new GuiceApplicationBuilder()
        ...
        .build()
        .injector
        .instanceOf[RefundRejectedController]
    ...
    "render the error page if the Rejected refund exists but doesn't include a completed date" in new TestSetup() {
      val fakeRequest: FakeRequest[AnyContentAsEmpty.type] =
        FakeRequest("GET", "/track-a-self-assessment-refund/refund-rejected")
          .withAuthToken()
          .withRequestId()
          .withSessionId()
        ...
        get(urlEqualTo(s"/self-assessment-refund-backend/repayments/${nino.value}/${no1.value}"))
          .willReturn(
            aResponse()
              ...
              .withBody(
                Json.prettyPrint(
                  Json.toJson(Response(no1, nino, 12000, "Rejected", "2021-08-14", None, None, Some("PO")))
                )
              )
          )
      )
      ...
      get(urlEqualTo(s"/self-assessment-refund-backend/repayments/${nino.value}/${key.value}"))
        .willReturn(
          aResponse()
            ...
            .withBody(
              Json.prettyPrint(
                Json.toJson(Response(key, nino, cashAmount, "Rejected", "2021-08-14", Some("2021-08-31"), None))
              )
            )
        )
    )
  ...
  private def checkRejectedPageContent(welsh: Boolean, agent: Boolean)(doc: Document): Unit = {
    val tryAgainLink =
      if (agent) "http://localhost:9081/report-quarterly/income-and-expenses/view/agents/money-in-your-account"
