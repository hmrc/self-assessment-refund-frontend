...
trait RepaymentConfirmationPageTesting extends PageContentTesting {
  def checkPageContent(
    ...
    doc.checkHasParagraphs(
      List(
        if (isCardOnFile && isAgent)
          s"We will send the refund to the card used to pay your client’s last Self Assessment tax bill. If we cannot do this, we will send the refund to the bank account ending in ${bankAccountNumber
              .takeRight(3)}."
        else if (isCardOnFile && !isAgent)
          s"We will send the refund to the card used to pay your last Self Assessment tax bill. If we cannot do this, we will send the refund to the bank account ending in ${bankAccountNumber
              .takeRight(3)}."
        else s"We will send the refund to the bank account ending in ${bankAccountNumber.takeRight(3)}.",
        if (isAgent)
          s"Your client should get their refund by $refundByDate. To protect you against fraud, HMRC has security measures in place which may cause a delay."
        else
          s"You or your agent should get the refund by $refundByDate. To protect you against fraud, HMRC has security measures in place which may cause a delay.",
    ...
    doc.checkHasHyperlink(
      ...
      "http://localhost:9171/track-a-self-assessment-refund/refund-request-tracker"
    )
    if (isAgent)
      doc.checkHasHyperlink(
        "contact us",
    ...
    if (isAgent)
      doc.checkHasActionAsButton("https://www.tax.service.gov.uk/agent-services-account/home", "Return to tax account")
  ...
  def checkPageContentWelsh(
    ...
    doc.checkHasParagraphs(
      List(
        if (isCardOnFile && isAgent)
          s"Byddwn yn anfon yr ad-daliad i’r cerdyn a ddefnyddiwyd i dalu bil treth Hunanasesiad diwethaf eich cleient. Os na allwn wneud hyn, byddwn yn anfon yr ad-daliad i’r cyfrif banc sy’n gorffen gyda ${bankAccountNumber
              .takeRight(3)}."
        else if (isCardOnFile && !isAgent)
          s"Byddwn yn anfon yr ad-daliad i’r cerdyn a ddefnyddiwyd i dalu’ch bil treth Hunanasesiad diwethaf. Os na allwn wneud hyn, byddwn yn anfon yr ad-daliad i’r cyfrif banc sy’n gorffen gyda ${bankAccountNumber
              .takeRight(3)}."
        else s"Byddwn yn anfon yr ad-daliad i’r cyfrif banc sy’n gorffen gyda ${bankAccountNumber.takeRight(3)}.",
        if (isAgent)
          s"Dylai’r ad-daliad ddod i law eich cleient erbyn $refundByDate. Er mwyn eich diogelu rhag twyll, mae gan CThEF fesurau diogelwch ar waith a allai achosi oedi."
        else
          s"Dylai’r ad-daliad ddod i’ch llaw, neu i law eich asiant, erbyn $refundByDate. Er mwyn eich diogelu rhag twyll, mae gan CThEF fesurau diogelwch ar waith a allai achosi oedi.",
    ...
    if (isAgent)
      doc.checkHasActionAsButton("https://www.tax.service.gov.uk/agent-services-account/home", "Yn ôl i’r cyfrif treth")
    doc.checkHasHyperlink(
      "Rydym yn defnyddio’ch adborth i wella ein gwasanaethau.",
      "/request-a-self-assessment-refund/feedback"
    )
    doc.checkHasHyperlink(
