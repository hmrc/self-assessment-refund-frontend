...
class BankAccountDetailsController @Inject() (
  ...
  private def alreadyVerifiedBankDetails(
  ...
  private def withFormData(isAgent: Boolean)(
    f: Form[BankAccount] => BankAccount => Future[Result]
  )(implicit messages: Messages, request: Request[_]): Future[Result] = {
    val fromRequest = BankAccountDetailsForm.form.bindFromRequest()
    fromRequest.fold(
      form =>
        Future.successful(
          BadRequest(accountPage(AccountPageModel(form, Seq.empty, isAgent)(messages))(messages, request))
        ),
      f(fromRequest)
    )
  }
  ...
  object AccountPageModel {
    ...
    def apply(bankDetails: Option[BankAccountInfo], isAgent: Boolean)(implicit messages: Messages): AccountPageModel = {
      val form = bankDetails
        .map(i => BankAccountDetailsForm.form.fill(i.mapTo[BankAccount]))
        .getOrElse(BankAccountDetailsForm.form)
    ...
    def apply(form: Form[BankAccount], fieldMessageOverrides: Seq[FormError], isAgent: Boolean)(implicit
      messages: Messages
    ): AccountPageModel =
      new AccountPageModel(
        pageTitle(form),
  ...
  implicit val conv: CMapping[BankAccount, BankAccountInfo] =
    (a: BankAccount) =>
      BankAccountInfo(
        ...
        a.rollNumber.map(RollNumber.apply)
      )
