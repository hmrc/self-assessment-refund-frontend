...
class SelectRepaymentAmountController @Inject() (
      ...
      case Some(amount) =>
        amount match {
          case Amount(
                ...
                tc @ Some(totalCreditAvailableForRepayment),
                Some(unallocatedCredit),
                suggestedRepaymentSelected
              ) if totalCreditAvailableForRepayment > 0 =>
            val model = SelectAmountPageModel(
              repayment,
              partialRepaymentSelected,
              totalCreditAvailableForRepayment,
              suggestedAmount = suggestedAmount(unallocatedCredit, totalCreditAvailableForRepayment),
              suggestedRepaymentSelected,
              request.isAgent
            )
            ...
            if (isAmountMatching) {
              ...
              auditService.auditRefundAmount(
                ...
                failureReason =
                  Some("totalCreditAvailableForRepayment does not match the value sent from view and change")
              )
              logAndReturnErrorPage(
                method = "selectAmount",
                s"[SelectRepaymentAmountController][selectAmount] Amounts from V&C and HIP-API#5277 are not the same, '${fullFromVC
                    .map(_.toString())
                    .getOrElse("missing")}' does not match '${totalCreditAvailableForRepayment.toString}'"
              )
            }
          case _ =>
            auditService.auditRefundAmount(
      ...
      case Some(amount) =>
        amount match {
          case Amount(_, _, _, Some(totalCreditAvailableForRepayment), Some(unallocatedCredit), _) =>
            ...
            def storeChosenAmount(): Future[Unit] =
              formData.get("choice").map(SelectAmountChoice.withNameLowercaseOnly) match {
                case Some(Suggested) =>
                  auditRefundAmount(amount.unallocatedCredit)
                  journeyConnector.setJourney(
                    journey.id,
                    journey.copy(amount = Some(amount.setSuggestedRepayment(amount.unallocatedCredit)))
                  )
                case Some(Full)      =>
                  auditRefundAmount(amount.totalCreditAvailableForRepayment)
                  journeyConnector.setJourney(
                    journey.id,
                    journey.copy(amount = Some(amount.setFullRepayment(amount.totalCreditAvailableForRepayment)))
                  )
                case Some(Partial)   =>
                  ...
                  journeyConnector.setJourney(
                    journey.id,
                    journey.copy(amount = Some(amount.setPartialRepayment(partialAmount)))
                  )
                case _               =>
                  sys.error(
                    s"[SelectRepaymentAmountController][storeChosenAmount] Data not found in the submitted form."
                  )
              }
    ...
    (request.journey.paymentMethod, redirectToCheckDetails) match {
      ...
      case (None, _)                           =>
        journeyConnector.lastPaymentMethod(request.journey.id).map { method =>
          ...
          logger.error(
            s"[SelectRepaymentAmountController][handleRedirect] Failed to retrieve payment method. Error: ${e.getMessage}"
          )
  ...
  private def agentRedirects(paymentMethod: PaymentMethod): Result =
    paymentMethod match {
      ...
      case BACS | PaymentOrder =>
        Redirect(controllers.refundRequestJourney.routes.WeNeedBankDetailsController.onPageLoadAgent)
  ...
  private def individualAndOrgRedirects(paymentMethod: PaymentMethod): Result =
    paymentMethod match {
      ...
      case BACS | PaymentOrder =>
        Redirect(controllers.refundRequestJourney.routes.WeNeedBankDetailsController.onPageLoad)
      case null                => sys.error("[SelectRepaymentAmountController][individualAndOrgRedirects] Unsupported payment method.")
    }

  private def logAndReturnErrorPage(method: String, message: String = "Not enough data in mongo to proceed.")(implicit
    request: BarsVerifiedRequest[_]
  ): Future[Result] = {
