  ...
  val postAccountType: Action[AnyContent] = actions.authenticatedRefundJourneyAction.async { implicit request =>
    withFormData(request.isAgent) { formAccountTypeEnum =>
      ...
      val isFromCheckDetailsPage        =
        request.session.get("self-assessment-refund.changing-account-from-cd-page").contains("redirectToCheckDetails")
      ...
      journeyConnector
        ...
        .map { _ =>
          Redirect(call).removingFromSession("self-assessment-refund.changing-account-from-cd-page")
        }
